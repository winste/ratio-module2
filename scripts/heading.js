function create(tag){  // создает HTML-элемент в зависимости от переданного тега
...
const createHeadline = () => {
  const heading = create("section").addClass('heading container').prependTo(document.querySelector("body")); /* создание элемента с помощью функции create(tag) и дальнейшее модификация
                                                                                                             с использованием методов в прототипе HTMLElement */
...
const createTimer = () => {
  const timer = create("div").addClass("heading__result-time timer").appendTo(document.querySelector(".heading__time"));  // создание общего блока, который будет содержать весь таймер
      timerTitle = create("p").addClass("heading__result-title timer__title").content("time: ").appendTo(timer);

      unitTime = create("div").addClass("heading__result-value timer__units").appendTo(timer);  // создание блока, в котором будут содержаться все единицы времени
...
const createBestTimeBLock = () => {
    const bestTimeCount = create("div").addClass("heading__result-time best-time").appendTo(document.querySelector(".heading__time"));  // общий блок
      bestTimeTitle = create("p").addClass("heading__result-title best-time__title").content("best: ").appendTo(bestTimeCount);
      bestTimeValue = create("p").addClass("heading__result-value best-time__value").addId("best-result").content(`${localStorage.getItem("best-time")  // при загрузке игры берется значение из LocalStorage
                                                                                        ? msToTime(localStorage.getItem("best-time")): 0}`)  // если значения нет, то вписать 0
...
// создание блока, считающего сумму все сложенных ячеек
const createScoreBLock = () => {
  const headingScore = document.querySelector(".heading__result");  // куда будет добавлен весь эелемент в итоге
      scoreCount = create("div").addClass("heading__count score").appendTo(headingScore); // создание общего блока для добавления подэлементов
